...
<dialog bind:this={dialog} onclose={() => (detail.open = false)} onclick={(e) => e.target === dialog && (detail.open = false)}>
  <div class="box">
    <div class="head">
      ...
      <button class="btn icon" aria-label="Tutup" onclick={() => (detail.open = false)}><Icon name="x" size={16} /></button>
    </div>
    <pre>{detail.text || 'Tidak ada detail tambahan.'}</pre>
    <div class="foot">
      <button class="btn" onclick={copy}><Icon name="copy" size={16} />Salin detail</button>
      <button class="btn-accent" onclick={() => (detail.open = false)}>Tutup</button>
    </div>
  </div>
</dialog>
